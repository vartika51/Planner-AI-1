...
section_choice = st.radio(
    "Choose Section:", ("Location Finder", "Trip Planner","Weather Forecasting"))
###########################################################################################
...
if section_choice == "Location Finder":
    ...
    #Prompt Template
    input_prompt_loc = """
    You are an expert Tourist Guide. As a expert your job is to provide summary about
...
if section_choice == "Trip Planner":
    ...
    You are an expert Tour Planner. Your job is to
    even if number of days is not provided.
    Also, suggest hidden secrets, hotels, and beautiful
    Also tell best month to visit given place.
    Retun the response using markdown.
    """

    #Input
    input_plan = st.text_area(
        "Provide location and number of days to obtain itinerary plan!")
    #Button
